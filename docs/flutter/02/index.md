...
縦はスクロールすればいいのですが、横幅が入らないのはあまり好ましくありません。横幅はおおむね360論理ピクセルを目安に作りましょう。
...
body: Column(
  children: [
    Container(
      padding: .all(20.0),
      margin: .only(bottom: 20.0),
      decoration: BoxDecoration(
        color: Colors.red,
        border: Border(right: BorderSide(width: 5.0, color: Colors.blue)),
      ),
      child: Text('test1', style: Theme.of(context).textTheme.bodyMedium),
    ),
    Text('test2', style: Theme.of(context).textTheme.bodyMedium),
    Text('test3', style: Theme.of(context).textTheme.bodyMedium),
  ],
),
```
...
複数の`Expanded`がある場合に、均等に割り付けずに割合を指定して余白を分配したい場合には`flex`に値を指定します。例えば、次の例の場合の1つ目の`Expanded`は`flex`が1です。これは、他の要素を含めての`flex`の合計が3です。
